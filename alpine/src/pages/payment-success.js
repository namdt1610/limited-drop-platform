// Payment Success Page (Web Component + Alpine)
import { PaymentSuccessLogic } from "./payment-success.logic.js";

// Định nghĩa custom element: <payment-success-page>
customElements.define(
  "payment-success-page",
  class extends HTMLElement {
    connectedCallback() {
      // Register Alpine data when element is connected
      if (!window.paymentSuccessDataRegistered) {
        Alpine.data("paymentSuccess", PaymentSuccessLogic);
        window.paymentSuccessDataRegistered = true;
      }

      this.innerHTML = /*html*/ `
<div x-data="paymentSuccess" x-init="init()" class="relative min-h-screen bg-black text-white font-inter selection:bg-white selection:text-black">
  <!-- Background -->
  ...
  <header class="reveal py-8 px-6 md:px-12 relative z-10">
    <div class="max-w-screen-2xl mx-auto flex items-center justify-between">
      ...
      <div class="flex flex-col items-end">
        ...
        <span class="text-xs font-bold uppercase tracking-widest">Xác nhận Giao dịch</span>
      </div>
    </div>
  </header>
      ...
      <div class="reveal py-12 md:py-20 space-y-12 text-center">
        ...
        <div class="space-y-6">
          <h1 class="text-5xl md:text-7xl font-spiky tracking-tighter leading-none uppercase">
            GIAO DỊCH<br/><span class="text-green-400">THÀNH CÔNG</span>
          </h1>
          <p class="text-lg text-white/60 leading-relaxed max-w-md mx-auto">
            Giao thức đã được xác nhận. Đơn hàng của bạn đã được ghi nhận vào hệ thống lõi.
          </p>
        </div>
        ...
        <div class="p-8 md:p-12 rounded-[2.5rem] bg-zinc-900/50 border border-white/10 backdrop-blur-3xl space-y-10 text-left">
          <div class="flex items-center gap-4">
            ...
            <h2 class="text-xs font-bold uppercase tracking-widest">Chi tiết Giao thức</h2>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-1">
              ...
              <p class="text-sm font-mono text-green-400 uppercase tracking-widest" x-text="isPaid ? 'Đã thanh toán' : 'Chờ xử lý'"></p>
            </div>
          ...
          <div class="pt-8 border-t border-white/5 space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
              ...
              <button @click="contactSupport" class="flex-1 py-6 rounded-full bg-white/5 border border-white/10 text-white text-[10px] font-bold tracking-[0.4em] uppercase transition-all duration-500 hover:bg-white/10">
                Liên hệ Hỗ trợ
              </button>
            </div>
          </div>
        ...
        <div class="space-y-4 text-center">
          <p class="text-[10px] font-mono text-white/20 uppercase tracking-[0.4em] leading-relaxed">
            ...
            <br/>Hệ thống đang chuẩn bị thực thể cho quá trình vận chuyển."
          </p>
        </div>
      </div>
    </div>
